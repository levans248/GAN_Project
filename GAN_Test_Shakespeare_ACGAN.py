from trainer_ACGAN import Trainer
from GAN_Shakespeare_ACGAN import GAN
...
# unique sentences
n_cats = np.unique(train_data).size
...
expt_name = "GAN_Shakespeare_ACGAN_Results"

create_directories(expt_name)
print("start GAN")
gan_ = GAN(norm_train.shape, n_cats)
print("start trainer")
trainer_ = Trainer(gan_, expt_name)
trainer_.train_gan(epochs=5000, batch_size=128, sample_interval=10, train_data=norm_train, train_labels=act_train_labels)
