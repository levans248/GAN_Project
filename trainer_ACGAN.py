...
class Trainer():
    ...
    def train_gan(self, epochs, batch_size, sample_interval, train_data, train_labels):
        ...
        for epoch in range(epochs):
            ...
            sampled_labels = real_seqs[1][0][0:batch_size]

            img_labels = train_labels[0:batch_size]


            # Generate batch of fake data using random noise
            noise = np.random.normal(0, 1, (batch_size, self.model.latent_dim))
            gen_seqs = self.model.generator.predict([noise, sampled_labels])

            # Train the discriminator to accept real data and reject fake data
            # d_loss_real = self.model.discriminator.train_on_batch(real_seqs, [real, sampled_labels])
            d_loss_real = self.model.discriminator.train_on_batch(real_seqs, [real, img_labels])
            d_loss_fake = self.model.discriminator.train_on_batch(gen_seqs, [fake, sampled_labels])
            ...
            noise = np.random.normal(0, 1, (batch_size, self.model.latent_dim))
            g_loss = self.model.gan.train_on_batch([noise, sampled_labels], [real, sampled_labels])
